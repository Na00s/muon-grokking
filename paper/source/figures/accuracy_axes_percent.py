"""Express accuracy axes as percentages while preserving original plot geometry.
...
def collapse(runs,w,h):
    ...
    for r in runs:
        ...
        if re.fullmatch(r'[01](?:\.\d+)?',t) and x<400 and r['size']>9:
            o.text(str(round(float(t)*100)) if y<180 else t,51.98 if x<100 else 387.65,y-1.3,anchor='right')
        ...
        elif t=='accuracy':o.text('accuracy (%)',18,128,anchor='center',rotate=90)
...
def seeds(runs,w,h):
    ...
    for r in runs:
        t,x,y=r['text'],r['x'],r['y']
        if t in ('0','0.5','1') and x<42:o.text(str(round(float(t)*100)),40.95,y-1.7,anchor='right')
        ...
        elif t=='test accuracy':o.text('test accuracy (%)',13,277,anchor='center',rotate=90)
...
def main():
    ...
    records=[]
    for name,builder,scale in [('figure1_collapse.pdf',collapse,.98*396/750),('figure3_seeds.pdf',seeds,.90*396/705.12)]:
        ...
        for r in runs:
            t=r['text']
            is_tick=(name=='figure1_collapse.pdf' and re.fullmatch(r'[01](?:\.\d+)?',t) and r['x']<400 and r['y']<180 and r['size']>9) or (name=='figure3_seeds.pdf' and t in ('0','0.5','1') and r['x']<42)
            if is_tick:
                r['text']=str(round(float(t)*100));conversions.append(dict(old=t,new=r['text'],x=r['x'],y=r['y']))
            elif (name=='figure1_collapse.pdf' and t=='accuracy') or (name=='figure3_seeds.pdf' and t=='test accuracy'):
                r['text']=t+' (%)';conversions.append(dict(old=t,new=r['text'],x=r['x'],y=r['y']))
        ...
        page.merge_page(overlay.finish());writer.add_metadata({'/Title':name.removesuffix('.pdf').replace('_',' '),'/Subject':'Accuracy axes in percent; original data and vector geometry preserved'})
        with out.open('wb') as f:writer.write(f)
        records.append(dict(figure=name,original='legibility_originals/'+name,original_sha256=sha(original),output_sha256=sha(out),canvas_pt=[w,h],canvas_unchanged=True,minimum_label_size_pt=overlay.size,minimum_printed_label_size_pt=overlay.size*scale,include_width_factor=scale,all_original_nontext_operations_unchanged=True,covered_area='Bottom legend strip only, y=0..28 PDF points',legend_swatches_redrawn=True,all_label_characters_and_numeric_tokens_preserved_after_registered_unit_conversion=True,accuracy_axis_conversions=conversions,text_show_operations_replaced=len(changed),labels=overlay.labels))
    result=dict(figures=records,font_sha256=sha(args.font),bold_font_sha256=sha(args.bold_font))
    (here/'accuracy_axis_revision_provenance.json').write_text(json.dumps(result,indent=2)+'\n')
