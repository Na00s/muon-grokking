"""Enlarge plot typography while retaining the original scientific vector artwork.
...
def revise(name, include_fraction, font_path):
    ...
    return dict(input=original.relative_to(HERE.parent).as_posix(), output=name,
                ...
                all_text_tokens_preserved=True, original_nontext_operations_preserved=True,
...
def main():
    ...
    (HERE / "projection_depth_legibility.json").write_text(json.dumps(records, indent=2) + "\n")
