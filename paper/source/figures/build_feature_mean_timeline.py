...
import os,csv,hashlib,json,math,re,tempfile
from pathlib import Path
os.environ.setdefault("MPLCONFIGDIR", str(Path(tempfile.gettempdir()) / "feature_mean_figure_cache"))
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
from matplotlib.lines import Line2D
from matplotlib.ticker import FuncFormatter
B=Path(__file__).resolve().parent;R=Path(os.environ.get('MUON_SOURCE_REPO',str(B.parents[2])))
...
fig=plt.figure(figsize=(5.5,4.2));gs=fig.add_gridspec(2,2,height_ratios=[2.5,1],hspace=.10,wspace=.30,left=.12,right=.955,top=.87,bottom=.18)
...
for j,(regime,title) in enumerate([('muon','Muon hidden / AdamW interface'),('adamw','Selected AdamW baseline')]):
 ...
 ax.text(.98,.04,f'{sum(x["regime"]==regime for x in data)} sampled checkpoints',ha='right',va='bottom',transform=ax.transAxes,fontsize=8)
