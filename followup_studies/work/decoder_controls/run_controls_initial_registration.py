...
def register():
    ...
    for seed, pair in enumerate(PAIRS):
        ...
        eligible = [r for r in rows if int(r['step']) % 100 == 0 and r['test_accuracy']
                    and float(r['train_accuracy']) >= .99 and float(r['test_accuracy']) < .95]
        selected = min(eligible, key=lambda r: int(r['step']))
    ...
    protocol = {'registered_at_utc': datetime.now(timezone.utc).isoformat(),
        ...
        'selection_rule': 'Choose the earliest recorded scheduled 100-step evaluation with native training accuracy at least 99% and native held-out accuracy below 95%, before sustained generalization. This operationalizes training-set memorization before grokking. Selection uses native trajectory metrics and is frozen before decoder fitting.',
...
def summarize():
    ...
    lines = ['# Initialization and memorization decoder controls', '',
        ...
        'The memorization checkpoint is the first scheduled 100-step evaluation with native training accuracy at least 99% and test accuracy below 95%. The selected steps are 200, 100, 100, 100 and 200 for seeds 0–4. These thresholds and identities were registered before fitting. Accuracies below are percentages.', '',
