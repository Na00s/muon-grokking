...
def continue_arm(event_directory, arm, steps):
    configure_runtime(1)
    ...
    assert untouched_other_parameters and untouched_optimizers
    ...
    meta = dict(event=metadata['event'], arm=arm, seed=source['seed'], start_step=source['step'], steps=steps,
                ...
                semantics='Readout-only robustness intervention with all saved optimizer moments preserved. Adam states are not transformed covariantly, so the continuation is not a gauge-equivalence proof.',
