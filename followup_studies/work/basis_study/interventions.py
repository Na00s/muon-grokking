...
def prepare_event(event, destination):
    ...
    if 'healthy_checkpoint' not in metadata and metadata.get('reference_role') == 'preceding_step':
        metadata.update(healthy_checkpoint=metadata['first_checkpoint'], collapsed_checkpoint=metadata['second_checkpoint'],
                        healthy_step=metadata['first_step'], collapsed_step=metadata['second_step'],
                        native_healthy=metadata['first_metrics'], native_collapsed=metadata['second_metrics'],
                        reference_field_semantics='Fields named healthy refer to the preceding-step reference and do not imply that checkpoint met a health threshold.')
