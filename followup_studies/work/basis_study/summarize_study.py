"""Generate the final study overview and report from saved numerical results."""
import csv
import json
...
def main():
    geometry={x['pair']:x for x in read(HERE/'geometry_summary.json')}
    bilinear={x['pair']:x for x in read(HERE/'bilinear'/'summary.json')}
    first_names={0:'seed0_dense_event',1:'seed1_first_event',2:'seed2_first_event',3:'seed3_first_event',4:'seed4_dense_event'}
    adjacent_names={0:'adjacent_seed0',1:'seed1_adjacent_event',2:'seed2_adjacent_event',3:'seed3_adjacent_event',4:'adjacent_seed4'}
    selected={0:EXP/'seed0_dense_event',1:HERE/'seed1_first_event',2:HERE/'seed2_first_event',3:HERE/'seed3_peak_event',4:EXP/'seed4_peak_event'}
    controls={0:'seed0_matched_healthy_16906_17200',1:'seed1_matched_healthy',2:'seed2_matched_healthy',3:'seed3_matched_healthy',4:'seed4_healthy_temporal_16044_16050'}
    overview=[]
    for seed in range(5):
        pair=selected[seed];m=read(pair/'metadata.json');p=read(pair/'decoder_whitened.json')
        g=geometry[first_names[seed]];a=bilinear[adjacent_names[seed]];c=geometry[controls[seed]]
        overview.append(dict(seed=seed,selected_pair=pair.name,selected_step=m['collapsed_step'],
            selected_native_accuracy=m['native_collapsed']['heldout']['accuracy'],
            selected_fresh_decoder_accuracy=p['cv_selected']['collapsed']['classification']['heldout']['accuracy'],
            reference_fresh_decoder_accuracy=p['cv_selected']['healthy']['classification']['heldout']['accuracy'],
            decoder_converged=p['cv_selected']['collapsed']['refit']['converged'],
            adjacent_preceding_accuracy=a['preceding_accuracy'],
            adjacent_feature_only_accuracy=a['feature_change_only_accuracy'],
            adjacent_readout_only_accuracy=a['readout_change_only_accuracy'],
            adjacent_actual_accuracy=a['actual_later_accuracy'],
            first_centered_backward_error=g['backward_centered_error'],
            healthy_matched_centered_backward_error=c['backward_centered_error'],
            adjacent_centered_backward_error=geometry[adjacent_names[seed]]['backward_centered_error'],
            adjacent_readout_norm_ratio=a['readout_change_norm_over_delta'],
            adjacent_feature_norm_ratio=a['feature_change_norm_over_delta']))
    baselines=[read(HERE/f'seed{i}_baseline'/'completion.json') for i in [1,2,3]]
    scope=dict(independent_seeds=5,fresh_seeds=[1,2,3],fresh_baseline_updates=sum(d['last_step'] for d in baselines),
        continuation_branches=18,continuation_updates=9000,instantaneous_evaluations=262,
        geometry_pairs=len(geometry),bilinear_pairs=len(bilinear),mathematical_tests=28,
        fresh_completion=baselines)
    (HERE/'results_overview.json').write_text(json.dumps(dict(scope=scope,seeds=overview),indent=2)+'\n')
    with (HERE/'results_overview.csv').open('w',newline='') as f:
        w=csv.DictWriter(f,fieldnames=list(overview[0]));w.writeheader();w.writerows(overview)
    swap_table=table(['Seed','Preceding model','Updated features + preceding readout','Preceding features + updated readout','Actual collapsed model'],
        [[d['seed'],*[pct(d[k]) for k in ['adjacent_preceding_accuracy','adjacent_feature_only_accuracy','adjacent_readout_only_accuracy','adjacent_actual_accuracy']]] for d in overview])
    decoder_table=table(['Seed','Collapsed update','Native test accuracy','Fresh linear decoder','Fresh decoder at reference'],
        [[d['seed'],d['selected_step'],pct(d['selected_native_accuracy']),pct(d['selected_fresh_decoder_accuracy']),pct(d['reference_fresh_decoder_accuracy'])] for d in overview])
    geometry_table=table(['Seed','First-event interval error','Matched-duration healthy error','Immediately preceding update error'],
        [[d['seed'],pct(d['first_centered_backward_error']),pct(d['healthy_matched_centered_backward_error']),pct(d['adjacent_centered_backward_error'])] for d in overview])
    report=f'''# Testing a pure change-of-basis account of post-grokking collapse
...
This study adds **{scope['fresh_baseline_updates']:,} baseline training updates across three fresh seeds**, **9,000 updates across 18 controlled continuations**, and **262 instantaneous head interventions**. All five examined seeds exhibit joint training and test collapse. There are {scope['geometry_pairs']} geometry comparisons and {scope['bilinear_pairs']} bilinear comparisons, including healthy and nearby-reference controls. Multiple snapshots and interventions from one seed are dependent observations.
...
'''
    ...
    print(json.dumps({'scope':scope,'report':str(HERE/'report.md')}))
