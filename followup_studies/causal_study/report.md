...
All five original trajectories have verified joint training/test failures by update 30,000. Accurate cross-entropy, introduced from the same saved post-grokking state at update 6,000, produces 0/5 such failures through update 30,000. Its lowest measured held-out accuracy across the complete branches is 98.859%, including their starting states. All five final accuracies are 100.000%.
...
|---|---:|---:|---|---:|---:|
| 0 | 17494 | 31.872% | None through 30,000 | 99.754% | 100.000% |
| 1 | 17721 | 9.095% | None through 30,000 | 99.843% | 100.000% |
| 2 | 18069 | 0.929% | None through 30,000 | 98.859% | 100.000% |
| 3 | 16308 | 53.597% | None through 30,000 | 99.978% | 100.000% |
| 4 | 16056 | 66.327% | None through 30,000 | 99.843% | 100.000% |
...
## Targeted derivative controls

In the common update 15,000 to 20,000 window, failure counts are 0/5 for accurate CE, 0/5 for correct-target-derivative repair, and 0/5 for per-example zero-sum projection. Each corresponding original trajectory has a verified failure inside that window. The three corrections retain the original float32 model and original Muon arithmetic.

| Seed | Accurate CE minimum test | Target repair minimum test | Zero-sum projection minimum test |
|---|---:|---:|---:|
| 0 | 99.978% | 99.978% | 100.000% |
| 1 | 99.989% | 99.989% | 100.000% |
| 2 | 100.000% | 100.000% | 100.000% |
| 3 | 99.978% | 99.978% | 100.000% |
| 4 | 100.000% | 100.000% | 100.000% |

Exact CE satisfies a zero class sum for each example's logit gradient. Target repair retains the original reported forward loss and every wrong-class derivative, reconstructing only the correct-class entry from the negative sum of the remaining entries. Zero-sum projection subtracts the class mean of the stock logit gradient. In the fixed seed 4 update 16,000 audit, target repair reduces relative gradient error from 32.02% to about 4.19e-7 (0.0000419%), while projection leaves 31.88%. The two interventions test different aspects of the observed error.

Removing the class-common gradient component prevents the registered joint failure in all five projection branches through update 20,000, while leaving most fixed-logit derivative error in the measured example. This intervention supports a consequential role for the broken class-sum identity in this window. Agreement with target-only repair and accurate CE strengthens the arithmetic-specific interpretation. These are finite, intervention-dependent results; they do not identify a unique mediator of the entire training history.
...
## Operation and architecture controls

The matched subtraction pair also attained sustained grokking. The original-CE arm first failed at update 15281, while the accurate-CE arm had no joint failure through update 30,000. Final subtraction test accuracies were 100.000% and 99.966%. The stock-CE RMS-normalized addition arm qualified for grokking at update 15700 and ended at 100.000% held-out accuracy. Its first joint failure occurred at update 18515. The accurate-CE RMS-normalized addition arm qualified for grokking at update 19900 and ended at 100.000% held-out accuracy. Its first joint failure occurred at update 28495.

| Condition | Arithmetic | Grokking confirmation | First joint failure | Minimum measured post-grok test | Final train | Final test |
|---|---|---:|---|---:|---:|---:|
| subtraction | stock | 9300 | 15281 | 5.851% | 100.000% | 100.000% |
| subtraction | accurate | 10500 | None through 30,000 | 96.879% | 100.000% | 99.966% |
| rms | stock | 15700 | 18515 | 0.481% | 100.000% | 100.000% |
| rms | accurate | 19900 | 28495 | 0.526% | 100.000% | 100.000% |

The subtraction failure reproduces the acute mechanism: actual test accuracy falls to 5.85%; the preceding head with updated features gives 98.81%; the head update alone gives 6.28%. Its mean contribution alone gives 5.84%, while the centered contribution retains 98.76%. Exact next-update replay passes. Each generality condition has one seed. Hyperparameters and horizons were fixed before these runs.
